import sql, { ConnectionPool, config as TypeConfig } from "mssql";

const pools = new Map();

export function db(name: "local" | "remote"): Promise<ConnectionPool> {
  /**
  ...
  const config: { local: TypeConfig; remote: TypeConfig } = {
    local: {
      ...
      options: {
        ...
        trustServerCertificate:
          global.AppConfig?.ambiente.local.options.trustServerCertificate ||
          true,
    ...
    remote: {
      ...
      options: {
        ...
        trustServerCertificate:
          global.AppConfig?.ambiente.producao.options.trustServerCertificate ||
          true,
